...
import {
  ...
  geo_types, editLinestringCfg, baseTextMarkerCfg, bgNormalOptIndx, activeOptIdx,
  normalOptIdx, h_type_text, h_type_number, h_type_date, source_customer,
  mapServerPointConfig, mapServerLineConfig, mapServerPolygonConfig, mapServerPointToPointConfig,
  mapServerPointToPolygonConfig, buffer_types, bgNormalOpt, activeOpt, normalOpt
} from "../../../constants/constants";
import { iconMap } from "../../../utils/iconUtil";
import bmapUtil from "../../../utils/bmapUtil";

const uuidv1 = require("uuid/v1");
const { polyOptA2S } = bmapUtil;
...
const PreStyle = superclass => class PreStyle extends superclass {
   ...
   */
  getBaseStyle = ({ geometry_type, robj, color, filter, from_to }) => {
  ...
  _prepareBgStyle = (node) => {
    ...
    let columns = [], layers = [], rules = [], styles = [], style;
    let uuid = uuidv1();
    let styleName = `poly_style_${uuid}`;
    layers.push({
      name: `poly_layer_${uuid}`,
    ...
    if (cfg && visualize_type === 1) {
      ...
      let types = other ? domain.slice(0, -1) : domain, typeArr = [];
  ...
  _prepareStaticBgStyle = (node, current) => {
    ...
    let { geo_filters: { filters } } = current;
    let filter = filters && filters[0] || {};
    let { cur_visual, hidden, style: robj, show_col } = filter;
    if (hidden) return; //如果隐藏则不请求
    let visual_cfg = null;
  ...
  _prepareStyle = (node, filter, geo_filter, from_to) => {
    ...
    let columns = [], rules = [], layers = [], styles = [], actions = [], postBody;
    ...
    // 图层
    let styleName = `poly_style_${uid}`;
    let layerName = `poly_layer_${uid}`;
    ...
    if (cfg) {
      ...
      if (visualize_type === 1) { //数值分段
        ...
        if (other) {
          let style = this.getBaseStyle({ geometry_type, robj, color: range[range.length - 1], filter, from_to });
          rules.push({
            "else_filter": true,
            ...style
          });
  ...
  _preSizeVisualStyle = (size_visual, actions, styles, columns, node) => {
    if (size_visual) {
      ...
      actions.push({
        "action": "min_max_scaler",
        "column_for_domain": isMarket ? "" : `extra.${size_visual.key}`,
      ...
      if (styles && styles[0] && styles[0].rules && styles[0].rules[0] && styles[0].rules[0].marker) {
        styles[0].rules[0].marker.width = "[_mapped_]";
      }
      let fd = find(columns, t => t[0] === size_visual.key);
      if (!fd) {
        columns.push([isMarket ? "" : `extra.${size_visual.key}`, size_visual.db]);
      }
    }
  };
